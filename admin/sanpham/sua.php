<?php 
    ...
    while ($kq_sua=mysqli_fetch_array($rs_sua)) {
        $sql_anhsp="SELECT * FROM `anhsp` WHERE MaSP=$masp";
        $rs_anhsp=mysqli_query($conn,$sql_anhsp);
        $kq_anhsp=mysqli_fetch_array($rs_anhsp);
?>
...
<form class="form-row " method="POST" action="sanpham/xuly.php" enctype="multipart/form-data">
    ...
    <div class="form-group col-sm-3">
        ...
        <select name="madm" class="form-control browser-default custom-select">
            ...
            <option <?php if($row['MaDM']===$kq_sua['MaDM']) { echo "selected";} ?> value="<?php echo $row['MaDM'] ?>">
                <?php echo $row['MaDM'].' - '.$row['TenDM']; ?></option><?php } ?>
        </select>
    </div>
    <div class="form-group col-sm-2">
        ...
        <select name="mancc" class="form-control browser-default custom-select">
            ...
            <option <?php if($row2['MaNCC']===$kq_sua['MaNCC']) { echo "selected";} ?>
                value="<?php echo $row2['MaNCC']; ?>"><?php echo $row2['MaNCC'].' - '.$row2['TenNCC']; ?></option>
            <?php } ?>
        </select>
    </div>
    <div class="form-group col-sm-2"><label>Đơn Giá</label><input type="text" class="form-control" name="dongia"
            value="<?php echo $kq_sua['DonGia'];?>"></div>
    <div class="form-group col-sm-4"><label>Ảnh Nền</label> <input type="file" class="form-control" name="anhnen">
    </div>
    <div class="form-group col-4"><label>Ảnh sản phẩm 1</label> <input type="file" class="form-control" name="anhsp1">
        <img src="../../webroot/image/sanpham/<?php echo $kq_anhsp['Anh1']; ?>" width="100">
    </div>
    <div class="form-group col-4"><label>Ảnh sản phẩm 2</label> <input type="file" class="form-control" name="anhsp2">
        <img src="../../webroot/image/sanpham/<?php echo $kq_anhsp['Anh2']; ?>" width="100">
    </div>
    <div class="form-group col-4"><label>Ảnh sản phẩm 3</label> <input type="file" class="form-control" name="anhsp3">
        <img src="../../webroot/image/sanpham/<?php echo $kq_anhsp['Anh3']; ?>" width="100">
    </div>
    <div class="form-group col-4"><label>Ảnh sản phẩm 4</label> <input type="file" class="form-control" name="anhsp4">
        <img src="../../webroot/image/sanpham/<?php echo $kq_anhsp['Anh4']; ?>" width="100">
    </div>
    <div class="form-group col-sm-8"><label>Mô Tả</label><textarea class="form-control" name="mota"
         ...
         while ($kq_mau=mysqli_fetch_array($rs_mau)){$mm=$kq_mau['MaMau'];$sql_mau2="select DISTINCT MaMau from chitietsanpham where MaSP=$masp and MaMau='$mm'";
            ...
            <div class=" custom-checkbox custom-control col-2 ">
                <input type="checkbox" class="custom-control-input" id="<?php echo $kq_mau['MaMau']; ?>" name="mau[]"
                    value="<?php echo $kq_mau['MaMau']; ?>"
                    <?php  while ($kq_mau2=mysqli_fetch_array($rs_mau2)) {if($kq_mau['MaMau']===$kq_mau2['MaMau']){ echo "checked";  }}?>>
